...
//        }
		Dtype a_forest = (Dtype) 0.0;
		for (int t = 0; t < num_trees_; t++) {
			Dtype a_tree = (Dtype) 0.0;
			int k = 0;
	        for (int i = 0; i < num_leaf_nodes_per_tree_; i++)
	        {
	      	  	a_tree += routing_leaf_prob_.data_at(0, 0, t, i) * 0.5 * (1 + log(2 * PI * sigma_square_->data_at(t, i, 0, 0)));
	        }
	        a_forest += a_tree;
		}
		of_ << "Entropy: " << a_forest / num_trees_ << "\n";
	}
